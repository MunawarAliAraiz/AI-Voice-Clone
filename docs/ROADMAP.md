...
| **YouTube transcript import** | ✅ Shipped and **browser-verified against a real video** (2026-08-17) | Paste a link, read the caption track, send it to the editor whole or a chunk at a time. Full design and reasoning: **[TRANSCRIPT_IMPORT.md](TRANSCRIPT_IMPORT.md)**. The load-bearing parts: `domain/youtube.parse_video_id` is an **SSRF guard** — the endpoint fetches server-side from a user-supplied string, so no URL is ever fetched, only an 11-char video id extracted from a `urlsplit`-checked hostname (a regex over the whole URL accepts `youtube.com.evil.test` and `www.youtube.com@evil.test`; ten such URLs are tested at the domain layer and five more through HTTP, asserting **nothing was fetched**, because a guard that rejects after making the request is not a guard). Cues join with a SPACE, newline only after real sentence punctuation including `۔`/`।` — caption tracks split one sentence across three cues, and `direction_analyze` now gives a newline the longest pause there is, so the naive join would have put ~380 ms of silence inside most sentences. Chunking reuses `chunk_for_synthesis` unchanged, with `max_chars` from config rather than a `ModelSpec` field nobody has measured. `needs_transliteration` is computed server-side from the catalog so the UI never encodes routing rules. Synchronous, not a job: the `jobs` table is the GPU queue and this never touches the GPU. yt-dlp is Unlicense (rule 6) and pure Python (rule 2). **Devanagari conversion is detected and refused, not attempted** — see Phase B below. **Two bugs a fixture could not have found, and a real fetch did.** `_choose` fell through to "the first authored track", which on a popular video is effectively alphabetical: an English lecture came back as its **Arabic translation** — fluent, plausible, and not one word of what the speaker said. The order is now preference → **the video's own language** → English → first-authored; the middle step is the fix. That same video carried **4867 tracks, 4837 of them machine auto-translations, at 367 KB of JSON** for a list no UI can present — `_selectable_tracks` keeps the authored ones plus whichever was chosen (included even when auto-generated, so the response can never describe a selection missing from its own list) and `total_tracks` carries the count away rather than losing it. Re-measured: 30 tracks, 40 KB. Send-to-editor verified end to end — the chunk lands in the Composer byte-identical and the app switches tabs. **Reworked 2026-08-18** for videos past a toy length: cues are now chunked **per chapter** (`parse_chapters`/`group_cues`, no chapters → byte-identical to before, `index` renumbered globally so a per-chapter `0` can't collide), the Import tab renders parts as a collapsible table of contents with a chapter jump list and a bulk-select bar (`useTranscriptParts`, source read-only vs editable conversion-under-review), and the free-text caption-language box was replaced with a **track picker** populated from `available_tracks` after the first fetch. `truncated` is now reported to the user rather than only logged. |
| **Phase B — Roman/Devanagari → Perso-Arabic transliterator** | 🟢 **Wired end to end, GPU-verified for `latin → perso_arabic`, and reachable from the Composer.** `.venv-gemma` provisioned on the A40 pod 2026-08-17; Gemma is now **resident** (idle-killed, warmed at startup). Only the **Devanagari** listening gate remains unrun. | The VRAM question is **answered and built**: `InferenceScheduler.exclusive_gpu(reason)` holds the main GPU slot and evicts every worker, so Gemma-4-31B (~19 GB against 24 GB) gets an empty card and no synthesis can run while it is resident. `TransliteratorScheduler` owns nothing between calls — spawn, LOAD, TRANSLITERATE, kill — because it is resident or the audio models are, never both; `_make_room_for` could not express this, since it evicts until a spec fits a budget sized for co-residency (16 GB) and a 19 GB spec fails its first check. **This is a second eviction call site and an explicit amendment to golden rule 3**, argued in `scheduler.py`'s docstring rather than smuggled in — the property the rule buys is intact because the same semaphore is held throughout and eviction still goes through the same `_evict` behind the same assertion. The validator (`domain/transliterate.py`) is pure and enforces that output is a *transliteration* and not an *answer*; its threshold was **measured, not chosen** — an intuition-picked 0.5 Perso-Arabic share rejected a correct code-switched conversion, and the real distribution is 0.115–1.000 for legitimate output versus exactly 0.000 for every failure, so the test is "did any conversion happen". One test exists purely to stop it being mistaken for a quality gate. **Wired to the API later the same day (2026-08-17):** `POST /api/text/transliterate` returns 202 and enqueues `JobKind.TRANSLITERATE`; the handler owns the ORDER (convert, then validate) and nothing else, which is what keeps "is this a transliteration or an answer" provable without a 19 GB download — its eight tests run against a fake scheduler. A rejection **fails the job** carrying the reason code and the text never reaches the client, because returning it with a warning attached is golden rule 5's silent substitution one layer above audio. `route=None`, like `analyze_llm`. Fixed at the same time: `GEMMA_TRANSLITERATOR_HF_REVISION` was the literal string `"main"` under a comment claiming it was pinned — golden rule 7's exact hole, made worse by a comment that stopped anyone looking — now `842da3794eaa…`, with the repo id's **capital B** restored (`gemma-4-31b-it` is a 307 redirect). **Devanagari as a source script also landed**, with its exemplars *derived* rather than written: the Urdu side of all six is byte-identical to the Roman set, because authoring new gold Urdu would put unreviewed Urdu into the PROMPT, where an error teaches the model the error instead of merely scoring badly. Presence, not dominance, picks the source set — `detect_script` returns MIXED for the ordinary shape of a Hindi caption carrying English words, and MIXED would fall back to Latin. The exemplars deliberately say **nothing** about an English loanword already spelled in Devanagari (मीटिंग); nothing has measured which OmniVoice says better, and an exemplar would be inventing the answer. **Since done (2026-08-17/18):** `.venv-gemma` provisioned and the `latin → perso_arabic` hop GPU-verified; Gemma made **resident** (~5 s re-convert, not ~78 s), which is what let the Roman-draft question be decided; and the Composer UI shipped as **client-side convert-on-generate** (Generate converts Roman→Urdu-script when the chosen model can't read Latin, shows a review step, one tap generates — `resolve()`/`TransformKind` untouched, golden rules 4/5). **Still to do:** the Devanagari **listening gate**, which does not exist and which the Import tab correctly still tells users about; and a browser pass of convert-on-generate on a warm pod. |
