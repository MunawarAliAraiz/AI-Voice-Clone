...
| **VoxCPM2 LoRA POC** | 🚧 Trained, evaluated, mixed result — merged to `main` (PR #12), no ship decision yet | Real LoRA fine-tune of VoxCPM2 on the owner's own voice (36 clips, 300 steps, ~15 min, no OOM). Baseline-vs-LoRA eval: CER improved sharply on Urdu (0.0818 → 0.0091) but speaker cosine regressed on Urdu (0.7226 → 0.6859, pass → fail) and only marginally improved on Hindi. Owner's own listen found the LoRA Urdu clip good despite the cosine regression — consistent with this project's known ECAPA-encoder unreliability, not a contradiction. Still not shipped, still no `LanguageSupport.verified` flag touched; a rigorous blind listen is the suggested next step. Full detail: [VOXCPM_LORA_POC.md](VOXCPM_LORA_POC.md). |
