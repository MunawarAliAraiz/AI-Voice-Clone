...
| **YouTube transcript import** | 🟡 Built and unit-tested 2026-08-17, **never exercised in a browser** | Paste a link, read the caption track, send it to the editor whole or a chunk at a time. Full design and reasoning: **[TRANSCRIPT_IMPORT.md](TRANSCRIPT_IMPORT.md)**. The load-bearing parts: `domain/youtube.parse_video_id` is an **SSRF guard** — the endpoint fetches server-side from a user-supplied string, so no URL is ever fetched, only an 11-char video id extracted from a `urlsplit`-checked hostname (a regex over the whole URL accepts `youtube.com.evil.test` and `www.youtube.com@evil.test`; ten such URLs are tested at the domain layer and five more through HTTP, asserting **nothing was fetched**, because a guard that rejects after making the request is not a guard). Cues join with a SPACE, newline only after real sentence punctuation including `۔`/`।` — caption tracks split one sentence across three cues, and `direction_analyze` now gives a newline the longest pause there is, so the naive join would have put ~380 ms of silence inside most sentences. Chunking reuses `chunk_for_synthesis` unchanged, with `max_chars` from config rather than a `ModelSpec` field nobody has measured. `needs_transliteration` is computed server-side from the catalog so the UI never encodes routing rules. Synchronous, not a job: the `jobs` table is the GPU queue and this never touches the GPU. yt-dlp is Unlicense (rule 6) and pure Python (rule 2). **Devanagari conversion is detected and refused, not attempted** — see Phase B below. |
| **Phase B — Roman/Devanagari → Perso-Arabic transliterator** | 🟡 **Foundation built, wired to NOTHING.** `exclusive_gpu` + validator + runtime + scheduler exist; endpoint, job kind, handler and UI do not | The VRAM question is **answered and built**: `InferenceScheduler.exclusive_gpu(reason)` holds the main GPU slot and evicts every worker, so Gemma-4-31B (~19 GB against 24 GB) gets an empty card and no synthesis can run while it is resident. `TransliteratorScheduler` owns nothing between calls — spawn, LOAD, TRANSLITERATE, kill — because it is resident or the audio models are, never both; `_make_room_for` could not express this, since it evicts until a spec fits a budget sized for co-residency (16 GB) and a 19 GB spec fails its first check. **This is a second eviction call site and an explicit amendment to golden rule 3**, argued in `scheduler.py`'s docstring rather than smuggled in — the property the rule buys is intact because the same semaphore is held throughout and eviction still goes through the same `_evict` behind the same assertion. The validator (`domain/transliterate.py`) is pure and enforces that output is a *transliteration* and not an *answer*; its threshold was **measured, not chosen** — an intuition-picked 0.5 Perso-Arabic share rejected a correct code-switched conversion, and the real distribution is 0.115–1.000 for legitimate output versus exactly 0.000 for every failure, so the test is "did any conversion happen". One test exists purely to stop it being mistaken for a quality gate. **Still to build:** `JobKind.TRANSLITERATE` + handler, `POST /api/text/transliterate`, config/lifespan wiring, `.venv-gemma`, Composer UI, and Devanagari as a source script — which has **no listening gate yet** and must not be presented as working until it passes one. |
