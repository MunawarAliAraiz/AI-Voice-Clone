...
| **Transliteration viability probe (Phase 2 of the Urdu plan)** | 🔴 **Ran twice, missed the gate both times (2026-08-15)** | `eval/run_translit_probe.py`, `Qwen/Qwen2.5-3B-Instruct` on the pod, scored Perso-Arabic→Devanagari and Roman→Devanagari against the corpus's hand-authored gold Devanagari. Mean CER 0.2771 / 0.3075, owner-sentence CER 0.15–0.48 — the plan's own gate ("close to gold or the transform layer isn't built") is not cleared, and the predicted Roman-easier-than-Perso-Arabic direction didn't hold either. Real errors seen went beyond vowel restoration (dropped words, semantic substitution, incomplete script-switching). Retried against the actually-correct target for OmniVoice — Roman→**Perso-Arabic** instead of Devanagari — via `eval/run_roman_arabic_probe.py`, same model, zero-shot and few-shot: zero-shot mean CER 0.2332 (owner_core ~0.23), few-shot 0.1942 among parseable cases but 2/5 owner_core items came back unparseable. Also misses the gate. **The `NoRouteError` on Roman Urdu → OmniVoice stays as-is** — no transform layer is being built on this model on either target script. Neither run rules out a bigger model or a dedicated non-LLM transliterator, but a web search (2026-08-15, no code written) found no adoptable non-LLM option: `urduhack` doesn't do Roman→Urdu conversion at all (general Urdu NLP toolkit; also Python 3.6/3.7-only) despite search summaries claiming otherwise; the two purpose-built candidates (`code-mj12/Roman-to-Urdu-Transliteration`, `irsaal/urdu`) are single-notebook academic projects, 4–18 stars, one GPL-3.0-licensed — and the underlying academic literature itself flags rule-based Roman→Urdu as fundamentally lossy (26 Latin letters mapping to ~44 Urdu sounds, no clean 1:1 rule set). A non-LLM transliterator is not the cheap win it looked like; a bigger LLM (e.g. `Qwen2.5-7B-Instruct` on the pod) is the more promising untried option if this line is pursued further. Full numbers and qualitative examples: [URDU_BAKEOFF_RESULTS.md §8](URDU_BAKEOFF_RESULTS.md#8-phase-2--transliteration-viability-probe-bench), manifests at `eval/results/roman_arabic_probe/manifest.json`. |
