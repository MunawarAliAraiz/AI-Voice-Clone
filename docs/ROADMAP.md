...
| **Roman Urdu → Perso-Arabic conversion (Phase 2 of the Urdu plan)** | 🟡 **Closed on Qwen 2026-08-16; a new model reopened the question the same day — awaiting a re-gate decision.** **The update first:** §13 re-ran A2 unchanged on `mistralai/Ministral-3-8B-Instruct-2512` (**Apache 2.0**, top of UrduMMLU's ≤25B open group) and measured **29/39 contract-clean (74%) at CER 0.0777**, versus the Qwen baseline's 46% / 0.2733 that A3 rejected — **82%** on the trusted original-13 items, **zero** unparseable responses across all four arms, and conversion completeness **1.0000** on three arms (Qwen's worst failure mode, Urdu left sitting in Latin, is simply absent). This does **not** reverse A3 — nobody has listened to Ministral's output, and text metrics have twice been shown here to be incapable of approving anything. It does mean the closure below was a verdict on Qwen, not on the idea, and a re-gate is cheap (`eval/run_a3_full_chain.py`, repoint `_A2_MANIFEST`/`_A2_ARM`). **The original closure, still accurate for what it tested:** Phase A ran end to end and returned four independent negatives: **A0** OmniVoice does not read Roman Urdu unaided (words right, accent English — CER was blind to it); **A2** the strongest Qwen baseline reaches only 46% contract-clean output, and §10a showed prompt engineering is not the lever; **A4** no adoptable purpose-trained model exists (the best, `Mavkif/m2m100_rup_rur_to_ur` at Char-BLEU 97.44, declares **no licence at all** → golden rule 6; the only MIT-licensed one, AI4Bharat IndicXlit, is word-level at 61% top-1 and needs `tensorflow~=2.2`); **A3** the owner listened to the full chain through the real `OmniVoiceBackend` and judged it *"not usable"*. **Phase B was never started — that was the plan's explicit gate.** `NoRouteError` on Roman Urdu → OmniVoice stands. The one cheap thing that would reopen it is the Mavkif repo gaining a licence (draft ready at `docs/outreach/mavkif-licence-request.md`); otherwise it needs a model trained on Roman-Urdu-Parl/Dakshina. Full reasoning per dead end: [URDU_BAKEOFF_RESULTS.md §9–§12](URDU_BAKEOFF_RESULTS.md). Two things this produced that outlive it: the corpus grew 13 → 45 items, and `eval/translit_metrics.py` (code-switch preservation + conversion completeness) exists for whenever a new candidate appears. |
