...
| **VoxCPM2 LoRA POC** | 🚧 Training succeeded, eval comparison never ran | Real LoRA fine-tune of VoxCPM2 on the owner's own voice (36 clips). 300 training steps completed cleanly (~15 min, no OOM) and the checkpoint was rescued off the pod before a scheduled shutdown — but the baseline-vs-LoRA cosine/CER/RTF comparison that actually decides whether it worked never ran. Merged to `main` (PR #11) as an honest partial checkpoint, not a finished result. Full detail: [VOXCPM_LORA_POC.md](VOXCPM_LORA_POC.md). |
