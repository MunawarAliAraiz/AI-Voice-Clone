...
| **Urdu bake-off + LoRA integration** | ✅ Shipped as an explicit experimental route | 10-arm controlled bake-off (VoxCPM2 × 4 representations, OmniVoice, Higgs v3 attempted-and-blocked) on a fixed 13-item corpus × 2 reference speakers, blind-listened 130/130 by the owner. Arm D (VoxCPM2 + the LoRA above, native Perso-Arabic, no transliteration) tied arm C (Devanagari transliteration) on quality and needed no new transliteration engineering, so it shipped: `voxcpm2_urdu_lora` spec, `experimental_listing=True` (mirrors Chatterbox — real positive listening scores, but the automated speaker-cosine gate doesn't clear 0.70 at either reference). Real pod smoke test: load (86.5s, LoRA applied with zero skipped weights) → synth (21.0s, 4.16s of audio, peak 0.92 — not silent) → unload, through the actual production `VoxCPMBackend` class. IndicF5 (arms H/I/J — the question of whether Devanagari transliteration generalizes to a *different* model) remains blocked on an `HF_TOKEN`. Full methodology, all ten arms' numbers, and the blind-listen breakdown: [URDU_BAKEOFF_RESULTS.md](URDU_BAKEOFF_RESULTS.md). Licensing per model: [URDU_MODEL_LICENSING.md](URDU_MODEL_LICENSING.md). |
